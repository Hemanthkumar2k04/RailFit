...
@router.get("/qr/{qr_code}", response_model=Asset)
async def get_asset_by_qr_code(
    qr_code: str,
    current_user: Dict[str, Any] = Depends(get_current_user_from_token)
):
    """Get asset by QR code - optimized for mobile QR scanning"""
    try:
        async with httpx.AsyncClient() as client:
            headers = {
                "apikey": SUPABASE_KEY,
            ...
            # Search by QR code field
            response = await client.get(
                f"{SUPABASE_URL}/rest/v1/assets",
                headers=headers,
                params={"qr_code": f"eq.{qr_code}"}
            )
            ...
            if response.status_code == 200:
                ...
                if assets:
                    ...
                    # Also try searching by asset_id in case QR code contains asset ID
                    response = await client.get(
                        f"{SUPABASE_URL}/rest/v1/assets",
                        headers=headers,
                        params={"asset_id": f"eq.{qr_code}"}
                    )
                    ...
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
                        detail=f"Asset not found for QR code: {qr_code}"
                    )
            else:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to search for asset"
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve asset by QR code: {str(e)}"
        )
